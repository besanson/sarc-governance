...
- [PAIS integration](docs/pais-integration.md) — `PAISContextMapper`, `PAISMemoryGuard`, preventing silent trace drops, full KAOS deployment guide.
- [MCP tool governance](docs/mcp-tool-governance.md) — governing MCP tools via the adapter pattern; scope, constraint examples, checklist.
...
**SARC governs explicitly wrapped tool boundaries only.** The adapter pattern makes the
governed boundary explicit: wrapping a `DelegationToolset` or MCP client governs every
...
**PAIS support** is available as a library module — not just a demo. See
...
| [`src/sarc_governance/adapters/`](src/sarc_governance/adapters/) | Optional framework adapters: `pais.py` — `PAISContextMapper`, `PAISMemoryGuard`, `build_governed_toolset` |
