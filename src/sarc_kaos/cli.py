"""sarc-kaos command-line interface.
...
``list-predicates``
    ...
    :mod:`sarc_kaos.predicates` (built-ins plus anything registered by
    importable plugins).
...
from sarc_kaos import audit_trace, predicates
from sarc_kaos.constraints import ConstraintSpec
...
def _run_procurement_demo() -> int:
    ...
    if not demo_path.exists():
        ...
        return 2
    spec = importlib.util.spec_from_file_location("_sarc_kaos_demo", demo_path)
...
def _version_string() -> str:
    try:
        from sarc_kaos import __version__  # local import to avoid circulars
    except Exception:  # pragma: no cover
        __version__ = "unknown"
    return f"sarc-kaos {__version__}"
